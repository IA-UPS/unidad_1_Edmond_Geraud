...
### NO estan asociados el genero y la obesidad ?
### p<0.05 si estan asociados, si el p.valor >0.05 no estan asociado

chisq.test(tabla)


normal.infra <- datos[grep("weight$",ignore.case = T,datos$NObeyesdad),]
### el genero no  esta relacionado con la hisotria familiar de la obesidad?

d$NObeyesdad <- as.factor(as.character(normal.infra$NObeyesdad))

table(datos$Gender,normal.infra$CAEC)


 datos %>% group_by(CAEC,Gender) %>% summarise(mean=mean(Age))
...
tabla <- table(datos$Gender,datos$family_history_with_overweight)
